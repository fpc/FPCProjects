...
Legal issues: Copyright (C) 1996-2003 by François PIETTE
...
const
  WSocketVersion            = 503;
  CopyRight    : String     = ' TWSocket (c) 1996-2003 Francois Piette V5.03 ';
...
{ which are _NOT_ freeware. See http://www.overbyte.be for details.         }
{$IFDEF USE_SSL}
  TCustomLineWSocket = class (TCustomSllWSocket)
{$ELSE}
...
begin
    ...
    if FType = SOCK_DGRAM then begin
        if FMultiCast then begin
             ...
             mreq.imr_multiaddr.s_addr :=  WSocket_inet_addr(szAddr);
...
begin
    ...
    if FSocksLevel[1] = '4' then begin
        ...
        PWORD(@Buf[2])^  := WSocket_ntohs(FPortNum);  { Dest port       }
