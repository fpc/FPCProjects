...
Legal issues: Copyright (C) 1998-2003 by François PIETTE
...
May 04, 2002  V1.23 Added "Len" argument to OnInlineDecodeLine event.
              ...
              arnaud_courrier@yahoo.fr who provided a test case.
              Made UUOutDec a little bit faster.
May 10, 2002  V1.24 Accept 'begin 644' as well as 'begin 666' for UUEncoding
              start. arnaud_courrier@yahoo.fr found that OE does that.
Nov 01, 2002  V1.25 Changed PChar arguments to Pointer to work around Delphi 7
              ...
              <arnaud_courrier@yahoo.fr> for providing his code.
...
const
    ...
    CopyRight : String = ' TMimeDecode (c) 1998-2003 Francois Piette V1.26 ';
...
begin
    ...
    Result   := FALSE;
    FileName := '';
    if CompareText(Copy(Line, 1, 6), 'begin ') = 0 then begin
        I := 7;
...
begin
    ...
    if (not FUUProcessFlag) and
       UUSectionBegin(S, cUUFilename) then begin
        out1 := LowerCase(S);
        if (Pos('--', out1) > 0) and (Pos('cut here', out1) > 0) then
            Exit;
        FUUProcessFlag := TRUE;
