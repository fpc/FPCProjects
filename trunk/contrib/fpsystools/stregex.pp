 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: needs minor syntax/typecast fix

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StRegEx.pas 1.01       *}
{*********************************************************}
{* SysTools: Regular Expression Handling                 *}
{*********************************************************}
...
uses
  SysUtils, Classes, StConst, StBase, StStrms;
...
type
  ...
  published
    ...
    property InLineTermChar : AnsiChar
      read FInLineTermChar
      write FInLineTermChar default #0;

    property InLineTerminator : TStLineTerminator
      read FInLineTerminator
      write FInLineTerminator default ltLf;
    ...
    property OutLineTerminator : TStLineTerminator
      read FOutLineTerminator
      write FOutLineTerminator default ltLf;
...
begin
  ////TL!!! This looks ok... I don't know why FPC doesn't like it: Result := IsCharAlpha(PChar(C));
end;
...
function TStRegEx.FileMasksToRegEx(Masks : AnsiString) : Boolean;
begin
  if (Assigned(FStream)) then begin
    SetStreamProperties;
