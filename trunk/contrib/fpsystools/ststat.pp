 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiled ok without any changes... ready for testing

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StStat.pas 1.01        *}
{*********************************************************}
{* SysTools: Statistical math functions modeled          *}
{*           on those in Excel                           *}
{*********************************************************}
...
uses
  {$IFDEF UseMathUnit} Math, {$ELSE} StMath, {$ENDIF}
...
function AveDevPrim(const Data; NData : Integer) : Double;
var
  i : Integer;
  m, s : Extended;
begin
...
function AveDev(const Data: array of Double) : Double;
begin
  Result := AveDevPrim(Data, High(Data)+1);
end;
...
function CorrelationPrim(const Data1, Data2; NData : Integer) : Double;
var
...
begin
  if (High(Data1) <> High(Data2)) then
    RaiseStatError(stscStatBadCount);
  Result := CorrelationPrim(Data1, Data2, High(Data1)+1);
end;

function CovariancePrim(const Data1, Data2; NData : Integer) : Double;
var
...
begin
  if (High(Data1) <> High(Data2)) then
    RaiseStatError(stscStatBadCount);
  Result := CovariancePrim(Data1, Data2, High(Data1)+1);
end;

function DevSqPrim(const Data; NData : Integer) : Double;
var
  i : Integer;
  sx, sxx, x : Extended;
begin
...
function DevSq(const Data: array of Double) : Double;
begin
  Result := DevSqPrim(Data, High(Data)+1);
end;

procedure FrequencyPrim(const Data; NData : Integer; const Bins; NBins : Integer;
  var Counts);
var
  b, i : Integer;
  Size : Cardinal;
...
begin
  if (High(Counts) <= High(Bins)) then
    RaiseStatError(stscStatBadCount);
  FrequencyPrim(Data, High(Data)+1, Bins, High(Bins)+1, Counts);
end;


function GeometricMeanPrim(const Data; NData : Integer) : Double;
var
  i : Integer;
  s, t : Extended;
begin
...
function GeometricMean(const Data: array of Double) : Double;
begin
  Result := GeometricMeanPrim(Data, High(Data)+1);
end;

function HarmonicMeanPrim(const Data; NData : Integer) : Double;
var
  i : Integer;
  s, t : Extended;
begin
...
function HarmonicMean(const Data: array of Double) : Double;
begin
  Result := HarmonicMeanPrim(Data, High(Data)+1);
end;


function LargestPrim(const Data; NData : Integer; K : Integer) : Double;
var
  b, t, i, j : integer;
  Size : LongInt;
  temp, pval : Double;
    ...
    while (t > b) do begin
      ...
      repeat
        ...
        if (i <= j) then begin
          temp := SD^[i];
          {$IFDEF VERSION2ONLY}                                        
          {need to do it this way because of a bug in the Delphi 2 and BCB 1}
          D := SD^[j];
          SD^[i] := D;
          {$ELSE}
          SD^[i] := SD^[j];
...
function Largest(const Data: array of Double; K : Integer) : Double;
begin
  Result := LargestPrim(Data, High(Data)+1, K);
end;
...
function MedianPrim(const Data; NData : Integer) : Double;
var
  m : Integer;
begin
  if (NData <= 0) then
  ...
  m := NData shr 1;
  if odd(NData) then
    Result := LargestPrim(Data, NData, m+1)
  else
    Result := (LargestPrim(Data, NData, m+1)+LargestPrim(Data, NData, m))/2.0;
end;

function Median(const Data: array of Double) : Double;
begin
  Result := MedianPrim(Data, High(Data)+1);
end;

function ModePrim(const Data; NData : Integer) : Double;
var
  maxf, i, f : Integer;
  Size : Cardinal;
  last, max : Double;
...
function Mode(const Data: array of Double) : Double;
begin
  Result := ModePrim(Data, High(Data)+1);
end;


function PercentilePrim(const Data; NData : Integer; K : Double) : Double;
const
  eps = 1.0e-10;
var
  ibin : Integer;
...
function Percentile(const Data: array of Double; K : Double) : Double;
begin
  Result := PercentilePrim(Data, High(Data)+1, K);
end;


function PercentRankPrim(const Data; NData : Integer; X : Double) : Double;
var
  b, t, m : Integer;
  Size : Cardinal;
  SD : PDoubleArray;
...
function PercentRank(const Data: array of Double; X : Double) : Double;
begin
  Result := PercentRankPrim(Data, High(Data)+1, X);
end;
...
function RankPrim(Number: Double; const Data; NData : Integer;
  Ascending: Boolean) : Integer;
var
  r : Integer;
begin
...
function Rank(Number: Double; const Data: array of Double;
  Ascending: Boolean) : Integer;
begin
  Result := RankPrim(Number, Data, High(Data)+1, Ascending);
end;

function SmallestPrim(const Data; NData : Integer; K : Integer) : Double;
var
  b, t, i, j : integer;
  Size : LongInt;
  temp, pval : Double;
...
function Smallest(const Data: array of Double; K : Integer) : Double;
begin
  Result := SmallestPrim(Data, High(Data)+1, K);
end;
...
function TrimMeanPrim(const Data; NData : Integer; Percent : Double) : Double;
var
  ntrim : Integer;
  Size : Cardinal;
  SD : PDoubleArray;
...
function TrimMean(const Data: array of Double; Percent : Double) : Double;
begin
  Result := TrimMeanPrim(Data, High(Data)+1, Percent);
end;

{------------------------------------------------------------------------}


procedure LinEstPrim(const KnownY; const KnownX; NData : Integer;
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  LinEstPrim(KnownY, KnownX, High(KnownY)+1, LF, ErrorStats);
end;


procedure LogEstPrim(const KnownY; const KnownX; NData : Integer;
  var LF : TStLinEst; ErrorStats : Boolean);
var
  i : Integer;
  Size : LongInt;
...
begin
  ...
  try
    ...
    LinEstPrim(lny^, KnownX, NData, LF, ErrorStats);
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  LogEstPrim(KnownY, KnownX, High(KnownY)+1, LF, ErrorStats);
end;


function ForecastPrim(X : Double; const KnownY; const KnownX;
  NData : Integer) : Double;
var
  LF : TStLinEst;
begin
  LinEstPrim(KnownY, KnownX, NData, LF, false);
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  Result := ForecastPrim(X, KnownY, KnownX, High(KnownY)+1);
end;

function ForecastExponentialPrim(X : Double; const KnownY; const KnownX;
  NData : Integer) : Double;
var
  LF : TStLinEst;
begin
  LogEstPrim(KnownY, KnownX, NData, LF, false);
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  Result := ForecastExponentialPrim(X, KnownY, KnownX, High(KnownY)+1);
end;


function InterceptPrim(const KnownY; const KnownX; NData : Integer) : Double;
var
  LF : TStLinEst;
begin
  LinEstPrim(KnownY, KnownX, NData, LF, false);
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  Result := InterceptPrim(KnownY, KnownX, High(KnownY)+1);
end;

function RSquaredPrim(const KnownY; const KnownX; NData : Integer) : Double;
var
  LF : TStLinEst;
begin
  LinEstPrim(KnownY, KnownX, NData, LF, true);
  Result := LF.R2;
end;
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  Result := RSquaredPrim(KnownY, KnownX, High(KnownY)+1);
end;

function SlopePrim(const KnownY; const KnownX; NData : Integer) : Double;
var
  LF : TStLinEst;
begin
  LinEstPrim(KnownY, KnownX, NData, LF, false);
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  Result := SlopePrim(KnownY, KnownX, High(KnownY)+1);
end;

function StandardErrorYPrim(const KnownY; const KnownX;
  NData : Integer) : Double;
var
  LF : TStLinEst;
begin
...
begin
  if (High(KnownY) <> High(KnownX)) then
    RaiseStatError(stscStatBadCount);
  Result := StandardErrorYPrim(KnownY, KnownX, High(KnownY)+1);
end;
