 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: commented out variant support... a few other fixes needed too

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StOStr.pas 1.01        *}
{*********************************************************}
...
{.Z+}
  ...
  protected
    ...
    ////TL! No variants in FPC? function  GetAsVariant : Variant;
    ////TL! No variants in FPC? procedure SetAsVariant(Value : Variant);
...
{.Z-}
  public
    ...
    ////TL! No variants in FPC? constructor CreateV(const S : Variant); virtual;
    ...
    procedure DeleteAtCursor(Length1 : Cardinal); ////TL Dupe id Length... added 1 suffix
    ...
    procedure StrStDelete(const Pos, Length1 : Cardinal); ////TL Dupe id Length... added 1 suffix
    ...
    ////TL! No variants in FPC? property AsVariant : Variant
    ////TL! No variants in FPC?   read GetAsVariant write SetAsVariant;
...
////TL! No FPC variants... removed constructor declaration
////TL! constructor TStString.CreateV(const S : Variant);
...
procedure TStString.DeleteAtCursor(Length1 : Cardinal); ////TL Dupe id Length... added 1 suffix
{- Deletes Length number of characters at cursor. }
begin
  StrStDeletePrimZ(FCursor, 0, Length1);
end;
...
procedure TStString.StrStDelete(const Pos, Length1 : Cardinal); ////TL Dupe id Length... added 1 suffix
...
begin
  AdjPos := Pos;
  if FOneBased then Dec(AdjPos);
  StrStDeletePrimZ(FString, AdjPos, Length1);
  FixCursor(AdjPos, Length1, True);
end;
...
begin
  FAlloc := FTempAlloc;
  ////TL!!! FCursor := (FCursor - FString) + FTemp;
  StrDispose(FString);
  FString := FTemp;
  FTemp := nil;
end;
...
////TL! No FPC variants... commenting out the following
////TL ! function TStString.GetAsVariant : Variant;
//{- Provide output as variant. }
//begin
  //Result := StrPas(FString);
//end;

////TL! procedure TStString.SetAsVariant(Value : Variant);
