...
{*********************************************************}
{* TurboPower SysTools for Kylix: StColl.pas 1.01        *}
{*********************************************************}
...
{Notes:
  ...
  - The page size is limited to 536 million elements in 32-bit mode.
...
procedure TStCollection.Assign(Source: TPersistent);
  begin
    ...
    {$ENDIF}
      ...
      //////////TL if not AssignPointers(Source, AssignData) then
      //////////TL  inherited Assign(Source);
    {$IFDEF ThreadSafe}
    finally
      LeaveCS;
    end;{try..finally}
...
begin
  ...
  if (PageElements = 0) {or
  (LongInt(PageElements)*SizeOf(Pointer) > MaxBlockSize)} then
