...
{*********************************************************}
{* TurboPower SysTools for Kylix: StTree.pas 1.01        *}
{*********************************************************}
...
procedure TStTree.Assign(Source: TPersistent);
  begin
    ...
    {$ENDIF}
      ...
      if not AssignPointers(Source, @AssignData) then ////TL added @ to AssignData
...
{$ENDIF}
    if conNodeProt = 0 then
      Iterate(@DestroyNode, True, nil); ////TL Added @
    trRoot := nil;
    FCount := 0;
{$IFDEF ThreadSafe}
  finally
...
{$ENDIF}
    ...
    T.Iterate(@JoinNode, True, Self); ////TL Added @
...
{$ENDIF}
    ...
    DisposeData := nil;
    OnDisposeData := nil;
    Iterate(@SplitTree, True, @SR);  ////TL Added @
...
{$ENDIF}
    ...
    try
      with Writer do begin
        ...
        Iterate(@StoreNode, false, @StoreInfo); ////TL Added @
        WriteListEnd;
      end;
    finally
      Writer.Free;
