 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   Initially adapted for use with Lazarus and FPC - 12-6-2003
   This unit was taken from the Delphi version of systools: wasn't in Kylix ver.
...
uses
  ////TL I always enjoy commenting this one out! :) Windows,
  SysUtils, Classes;
...
var
  ...
  Inx  : integer;
begin
  ////TL! EnterCriticalSection(SingletonLock);
  try
    dec(FRefCount);
    ...
    ////TL! LeaveCriticalSection(SingletonLock);
  end;
end;
{--------}
...
procedure InitUnit;
begin
  ////TL! InitializeCriticalSection(SingletonLock);
...
begin
  ...
  Instances.Free;
  ////TL! DeleteCriticalSection(SingletonLock);
end;
