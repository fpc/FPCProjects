 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: two unresolved problems in this unit... don't use until fixed.

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StEclpse.pas 1.01      *}
{*********************************************************}
{* SysTools: Calculating Lunar/Solar Eclipses            *}
{*********************************************************}
...
begin
  ...
  ///TL!!! DisposeData := DisposeEclipseRecord;
end;
...
{and/or annular but the axis of the moon's shadow does not touch the earth}
  if (I1 <> I2) and (I1 < 26) and (I2 < 26) then begin
    ...
    SolEc^.Path.DisposeData := @DisposePathData; ////TL added @ prefix
