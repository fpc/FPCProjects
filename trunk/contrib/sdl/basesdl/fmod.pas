...
function FSOUND_GetNumHardwareChannels: Integer; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_CD_Eject(Drive: Byte): ByteBool; {$IFDEF UNIX} cdecl {$ELSE} stdcall {$ENDIF};
...
const
  Default8087CW = $1332; //according to the FPC site it's the value used in the
                         //startup code.
procedure Set8087CW( value :word ); {$ifndef ver1_0}oldfpccall;{$endif} assembler;
asm
   FLDCW  value
end;
{$endif}
