...
type
  ...
  TSDLJoySticks = class( TSDLCustomInput )
  private
  ...
  public
    destructor Destroy; override;
    function UpdateInput( event: TSDL_EVENT ) : Boolean; override;
    function IsMouseDown( Button : Integer ) : Boolean;
    function IsMouseUp( Button : Integer ) : Boolean;
...
begin
  ...
  begin
    ...
    if ( event.button.button = SDL_BUTTON_WHEELUP ) then
      FOnMouseWheel( -1, SDL_GetModState, Point )
    else
      FOnMouseWheel( 1, SDL_GetModState, Point );
  end;
end;
...
function TSDLMouse.IsMouseDown(Button: Integer): Boolean;
begin
  SDL_PumpEvents;
...
function TSDLMouse.IsMouseUp(Button: Integer): Boolean;
begin
  SDL_PumpEvents;
