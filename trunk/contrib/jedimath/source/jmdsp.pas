{******************************************************************************}
{                                                                              }
{ JmDSP.pas for Jedi Math Alpha 1.02                                           }
...
{                                                                              }
{ This has been converted to use the other jedi maths units                    }
{                                                                              }
...
procedure ForwardFFT(const Source: array of TJmComplexRect; var Dest: array of TJmComplexRect; Count: integer);
...
procedure InverseFFT(const Source: array of TJmComplexRect; var Dest: array of TJmComplexRect; Count: integer);
...
{This module was written against a different complex unit than the jedi maths one}
{I have cheated a bit here and redirected the used methods to the jedi math ones}
{if this is a problem it will be easier to correct once we have some working test}
{cases.}


// Set a complex number
function Complex(Re: TjmFloat; Im: TjmFloat): TJmComplexRect;
begin
  result.Re := Re;
  result.Im := Im;
end;
...
function ComplexAdd(const C1, C2: TJmComplexRect): TJmComplexRect;
begin
  result := CAddRect(C1, C2);
end;

// Substract complex numbers (Result = C1 - C2)
function ComplexSub(const C1, C2: TJmComplexRect): TJmComplexRect;
begin
  result := CSubRect(C1, C2);
end;

// Multiply complex numbers (Result = C1 * C2)
function ComplexMul(const C1, C2: TJmComplexRect): TJmComplexRect;
begin
  result := CMultRect(C1, C2);
end;

// Scale complex numbers (Result = Scale * C)
function ComplexScl(Scale: TjmFloat; const C: TJmComplexRect): TJmComplexRect;
begin
  result := CMultRect(Complex(Scale, 0), C);
end;

// Get the magnitude of the complex number C
function ComplexMag(const C: TJmComplexRect): TjmFloat;
var
  CP : TJmComplexPolar;
begin
  CP := CRect2Polar( C );
...
function ComplexPhase(const C: TjmComplexRect): TjmFloat;
var
  CP : TJmComplexPolar;
begin
  CP := CRect2Polar( C );
...
const
  ...
  c31: TjmFloat = -1.5000000000000E+00; //  cos(2*pi / 3) - 1;
  c32: TjmFloat =  8.6602540378444E-01; //  sin(2*pi / 3);

  u5:  TjmFloat =  1.2566370614359E+00; //  2*pi / 5;
  c51: TjmFloat = -1.2500000000000E+00; // (cos(u5) + cos(2*u5))/2 - 1;
  c52: TjmFloat =  5.5901699437495E-01; // (cos(u5) - cos(2*u5))/2;
  c53: TjmFloat = -9.5105651629515E-01; //- sin(u5);
  c54: TjmFloat = -1.5388417685876E+00; //-(sin(u5) + sin(2*u5));
  c55: TjmFloat =  3.6327126400268E-01; // (sin(u5) - sin(2*u5));
  c8:  TjmFloat =  7.0710678118655E-01; //  1 / sqrt(2);
...
procedure ReorderSeries(Count: integer; var Factors: TIdx1FactorArray; var Remain: TIdx0FactorArray;
  const X: array of TJmComplexRect; var Y: array of TjmComplexRect);
var
  i, j, k: integer;
...
procedure FFT_2(var Z: array of TJmComplexRect);
var
  T1: TJmComplexRect;
begin
...
procedure FFT_3(var Z: array of TJmComplexRect);
var
  T1, M1, M2, S1: TJmComplexRect;
begin
...
procedure FFT_4(var Z: array of TJmComplexRect);
var
  T1, T2, M2, M3: TJmComplexRect;
begin
...
procedure FFT_5(var Z: array of TJmComplexRect);
var
  T1, T2, T3, T4, T5: TJmComplexRect;
  M1, M2, M3, M4, M5: TJmComplexRect;
  S1, S2, S3, S4, S5: TJmComplexRect;
begin
...
procedure FFT_8(var Z: array of TJmComplexRect);
var
  A, B: array[0..3] of TJmComplexRect;
  Gem: TjmFloat;
begin
  A[0] := Z[0]; B[0] := Z[1];
  A[1] := Z[2]; B[1] := Z[3];
...
procedure FFT_10(var Z: array of TJmComplexRect);
var
  A, B: array[0..4] of TJmComplexRect;
begin
   A[0] := Z[0];  B[0] := Z[5];
   A[1] := Z[2];  B[1] := Z[7];
   A[2] := Z[4];  B[2] := Z[9];
...
procedure SynthesizeFFT(Sofar, Radix, Remain: integer; var Y: array of TJmComplexRect);
var
  GroupOffset, DataOffset, Position: integer;
  GroupNo, DataNo, BlockNo, SynthNo: integer;
  Omega: double;
  S, CosSin: TJmComplexRect;
  Synth, Trig, Z: array[0..cMaxPrimeFactor - 1] of TJmComplexRect;
  ...
  var
    ...
    Re, Im: TJmComplexRect;
    V, W: array[0..cMaxPrimeFactorDiv2 - 1] of TJmComplexRect;
  begin
    N := Radix;
    AMax := (N + 1) div 2;
...
type
  ...
  TComplexArray = array[0..0] of TJmComplexRect;
var
  i: integer;
  FactorCount: integer;
...
begin
  ...
  if @Dest = @Source then begin
    GetMem(TmpDest, SizeOf(TJmComplexRect) * Count);;
    Move(Dest, TmpDest^, SizeOf(TJmComplexRect) * Count);
  end else begin
    TmpDest := @Dest;
  end;
  ...
  if @Dest = @Source then begin
    Move(TmpDest^, Dest, SizeOf(TJmComplexRect) * Count);
    FreeMem(TmpDest);
  end;
...
var
  i: integer;
  S: TjmFloat;
  TmpSource: array of TJmComplexRect;
begin
  if Count = 0 then exit;
