...
type
  ...
  TClient = class
   private
    procedure OnConnect(Sender: TLBaseFTPClient);
    procedure OnReceive(Sender: TLBaseFTPClient);
    procedure OnControl(Sender: TLBaseFTPClient);
    procedure OnSent(Sender: TLBaseFTPClient; const Bytes: Integer);
...
procedure TClient.OnConnect(Sender: TLBaseFTPClient);
begin
  FConnected:=True;
  Writeln('Connected succesfuly');
end;

procedure TClient.OnReceive(Sender: TLBaseFTPClient);
const
  BUFFER_SIZE = 65535;
var
  n: Integer;
...
procedure TClient.OnControl(Sender: TLBaseFTPClient);
var
  s: string;
begin
...
procedure TClient.OnSent(Sender: TLBaseFTPClient; const Bytes: Integer);
begin
  Write('.');
end;
