...
type
  ...
  TLBaseFTPClient = class;
                 
  TLFTPClientProgressCallback = procedure (Sender: TLBaseFTPClient; const Bytes: Integer) of object;

  TLFTPClientCallback = procedure (Sender: TLBaseFTPClient) of object;
  ...
  { TLBaseFTPTelnetClient }
  TLBaseFTPTelnetClient = class(TLBaseTelnetClient)
   protected
  ...
  TLBaseFTPClient = class(TComponent, ILClient)
   protected
    FControl: TLBaseTelnetClient;
    FData: TLBaseTcp;//TLTcpList;
   ...
   public
    ...
    function Connect(const HostName: string; const Port: Word = 21): Boolean; virtual;
   ...
   public
    ...
    property ControlConnection: TLBaseTelnetClient read FControl;
    property DataConnection: TLBaseTCP read FData;
  ...
  TLFTPClient = class(TLBaseFTPClient)
   protected
    procedure MakeConnections(SocketClass: TLSocketClass); override;
   public
    function Connect(const HostName: string; const Port: Word = 21): Boolean; override;
...
procedure TLBaseFTPTelnetClient.React(const Operation, Command: Char);
begin
...
constructor TLBaseFTPClient.Create(aOwner: TComponent);
begin
  Create(aOwner, nil);
end;

constructor TLBaseFTPClient.Create(aOwner: TComponent; SocketClass: TLSocketClass);
var
  s: TLFTPStatus;
begin
...
destructor TLBaseFTPClient.Destroy;
begin
  Disconnect;
  FSL.Free;
  FControl.Free;
...
function TLBaseFTPClient.GetTransfer: Boolean;
begin
  Result:=FData.Connected;
end;

function TLBaseFTPClient.GetEcho: Boolean;
begin
  Result:=FControl.OptionIsSet(TS_ECHO);
end;

function TLBaseFTPClient.GetConnected: Boolean;
begin
  Result:=FStatusFlags[fsCon] and FControl.Connected;
end;

function TLBaseFTPClient.GetBinary: Boolean;
begin
  Result:=FStatusFlags[fsType];
end;

function TLBaseFTPClient.CanContinue(const aStatus: TLFTPStatus; const Arg1,
  Arg2: string): Boolean;
begin
...
function TLBaseFTPClient.CleanInput(var s: string): Integer;
var
  i: Integer;
begin
  FSL.Text:=s;
...
procedure TLBaseFTPClient.SetStartPor(const Value: Word);
begin
  FStartPort:=Value;
...
procedure TLBaseFTPClient.SetEcho(const Value: Boolean);
begin
  if Value then
...
procedure TLBaseFTPClient.SetBinary(const Value: Boolean);
const
...
procedure TLBaseFTPClient.OnCo(aSocket: TLSocket);
begin
  if Assigned(FOnConnect) then
    FOnConnect(Self);
end;

procedure TLBaseFTPClient.OnEr(const msg: string; aSocket: TLSocket);
begin
  FSending:=False;
...
procedure TLBaseFTPClient.OnRe(aSocket: TLSocket);
begin
  if Assigned(FOnReceive) then
    FOnReceive(Self);
end;

procedure TLBaseFTPClient.OnDs(aSocket: TLSocket);
begin
...
procedure TLBaseFTPClient.OnSe(aSocket: TLSocket);
begin
  if Connected and FSending then
    SendChunk(True);
end;

procedure TLBaseFTPClient.OnControlRe(aSocket: TLSocket);
begin
  if Assigned(FOnControl) then
    FOnControl(Self);
end;

procedure TLBaseFTPClient.EvaluateAnswer(const Ans: string);
...
procedure TLBaseFTPClient.PasvPort;
...
procedure TLBaseFTPClient.SendChunk(const CallBack: Boolean);
const
  BUFFER_SIZE = 65535;
var
...
procedure TLBaseFTPClient.ExecuteFrontCommand;
begin
...
procedure TLBaseFTPClient.MakeConnections(SocketClass: TLSocketClass);
begin
  FData:=TLBaseTcp.Create(nil, SocketClass);
  FControl:=TLBaseFTPTelnetClient.Create(nil, SocketClass);
end;

function TLBaseFTPClient.Get(var aData; const aSize: Integer; aSocket: TLSocket): Integer;
var
  s: string;
begin
...
function TLBaseFTPClient.GetMessage(out msg: string; aSocket: TLSocket): Integer;
begin
...
function TLBaseFTPClient.Send(const aData; const aSize: Integer; aSocket: TLSocket
  ): Integer;
begin
  Result:=FControl.Send(aData, aSize);
end;

function TLBaseFTPClient.SendMessage(const msg: string; aSocket: TLSocket
  ): Integer;
begin
  Result:=FControl.SendMessage(msg);
end;

function TLBaseFTPClient.GetData(var aData; const aSize: Integer): Integer;
begin
  Result:=FData.Iterator.Get(aData, aSize);
end;

function TLBaseFTPClient.GetDataMessage: string;
begin
  Result:='';
...
function TLBaseFTPClient.Connect(const HostName: string; const Port: Word): Boolean;
begin
  Result:=False;
  Disconnect;
...
function TLBaseFTPClient.Authenticate(const aUsername, aPassword: string): Boolean;
begin
...
function TLBaseFTPClient.Retrieve(const FileName: string): Boolean;
begin
  Result:=not FPipeLine;
...
function TLBaseFTPClient.Put(const FileName: string): Boolean;
begin
  Result:=not FPipeLine;
...
function TLBaseFTPClient.ChangeDirectory(const DestPath: string): Boolean;
begin
  Result:=not FPipeLine;
...
function TLBaseFTPClient.MakeDirectory(const DirName: string): Boolean;
begin
  Result:=not FPipeLine;
...
function TLBaseFTPClient.RemoveDirectory(const DirName: string): Boolean;
begin
  Result:=not FPipeLine;
...
function TLBaseFTPClient.DeleteFile(const FileName: string): Boolean;
begin
  Result:=not FPipeLine;
...
function TLBaseFTPClient.Rename(const FromName, ToName: string): Boolean;
begin
  Result:=not FPipeLine;
...
procedure TLBaseFTPClient.List(const FileName: string = '');
begin
...
procedure TLBaseFTPClient.Nlst(const FileName: string);
begin
  if CanContinue(fsList, FileName, '') then begin
    PasvPort;
    FStatus.Insert(MakeStatusRec(fsList, '', ''));
...
procedure TLFTPClient.MakeConnections(SocketClass: TLSocketClass);
begin
  FData:=TLTcp.Create(nil, SocketClass);
  FControl:=TLFTPTelnetClient.Create(nil, SocketClass);
end;

function TLFTPClient.Connect(const HostName: string; const Port: Word
  ): Boolean;
begin
  Result:=inherited Connect(HostName, Port);
