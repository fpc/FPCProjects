...
type
  ...
  TLUdp = class(TLBaseUdp)
   protected
    procedure FreeSocks; override;
   public
    function Connect(const Address: string; const APort: Word): Boolean; override;
    function Listen(const APort: Word): Boolean; override;
...
constructor TLBaseUdp.Create(aOwner: TComponent);
begin
  Create(aOwner, nil)
end;

constructor TLBaseUdp.Create(aOwner: TComponent; SocketClass: TLSocketClass);
begin
...
procedure TLBaseUdp.Disconnect;
begin
  FRootSock.Disconnect;
end;

function TLBaseUdp.Listen(const APort: Word): Boolean;
begin
  Result:=False;
...
function TLBaseUdp.Connect(const Address: string; const APort: Word): Boolean;
var
  IP: string;
begin
  Result:=False;
...
procedure TLBaseUdp.Bail(const msg: string);
begin
  Disconnect;
  if Assigned(FOnError) then
...
procedure TLBaseUdp.SetCallbacks(aSocket: TLSocket);
begin
...
procedure TLBaseUdp.ReceiveAction(aSocket: TLHandle);
begin
...
procedure TLBaseUdp.SendAction(aSocket: TLHandle);
begin
  with TLSocket(aSocket) do begin
    FCanSend:=True;
    FIgnoreWrite:=True;
...
procedure TLBaseUdp.ErrorAction(aSocket: TLHandle);
begin
  Bail('Error' + LStrError(LSocketError));
end;

function TLBaseUdp.IterNext: Boolean;
begin
  Result:=False;
end;

procedure TLBaseUdp.IterReset;
begin
end;

function TLBaseUdp.GetConnected: Boolean;
begin
  Result:=FRootSock.Connected;
end;

function TLBaseUdp.Get(var aData; const aSize: Integer; aSocket: TLSocket): Integer;
begin
  Result:=FRootSock.Get(aData, aSize);
end;

function TLBaseUdp.GetMessage(out msg: string; aSocket: TLSocket): Integer;
begin
  Result:=FRootSock.GetMessage(msg);
end;

function TLBaseUdp.SendMessage(const msg: string; aSocket: TLSocket = nil): Integer;
begin
  Result:=FRootSock.SendMessage(msg)
end;

function TLBaseUdp.SendMessage(const msg: string; const Address: string): Integer;
begin
  FRootSock.FPeerAddress.Addr:=StrToNetAddr(Address);
  Result:=FRootSock.SendMessage(msg)
end;

function TLBaseUdp.Send(const aData; const aSize: Integer; aSocket: TLSocket): Integer;
begin
  Result:=FRootSock.Send(aData, aSize)
end;

function TLBaseUdp.Send(const aData; const aSize: Integer; const Address: string
  ): Integer;
begin
  FRootSock.FPeerAddress.Addr:=StrToNetAddr(Address);
  Result:=FRootSock.Send(aData, aSize);
...
constructor TLBaseTcp.Create(aOwner: TComponent);
begin
  Create(aOwner, nil)
end;

constructor TLBaseTcp.Create(aOwner: TComponent; SocketClass: TLSocketClass);
begin
  inherited Create(aOwner, SocketClass);
  FIterator:=nil;
  FCount:=0;
...
function TLBaseTcp.Connect(const Address: string; const APort: Word): Boolean;
begin
  Result:=False;
  if Assigned(FRootSock) then
    Disconnect;
...
function TLBaseTcp.Listen(const aPort: Word): Boolean;
begin
  Result:=false;
...
procedure TLBaseTcp.Bail(const msg: string; aSocket: TLSocket);
begin
  if Assigned(FOnError) then
...
function TLBaseTcp.IterNext: Boolean;
begin
  Result:=False;
...
procedure TLBaseTcp.Disconnect;
var
...
procedure TLBaseTcp.DisconnectSocket(var aSocket: TLSocket);
begin
...
procedure TLBaseTcp.ConnectAction(aSocket: TLHandle);
var
  {$ifdef MSWINDOWS}
  a: TSockAddrin;
  {$else}
...
procedure TLBaseTcp.AcceptAction(aSocket: TLHandle);
var
  Tmp: TLSocket;
begin
...
procedure TLBaseTcp.ReceiveAction(aSocket: TLHandle);
begin
...
procedure TLBaseTcp.SendAction(aSocket: TLHandle);
begin
  with TLSocket(aSocket) do begin
    if Connecting then
...
procedure TLBaseTcp.ErrorAction(aSocket: TLHandle);
begin
  with TLSocket(aSocket) do begin
    if Connecting then
      Self.Bail('Error on connect: connection refused' , TLSocket(aSocket))
...
function TLBaseTcp.GetConnected: Boolean;
var
  Tmp: TLSocket;
begin
  Result:=False;
...
function TLBaseTcp.Get(var aData; const aSize: Integer; aSocket: TLSocket): Integer;
begin
  Result:=0;
...
function TLBaseTcp.GetMessage(out msg: string; aSocket: TLSocket): Integer;
begin
  Result:=0;
  if not Assigned(aSocket) then
    aSocket:=FIterator;
...
function TLBaseTcp.Send(const aData; const aSize: Integer; aSocket: TLSocket): Integer;
begin
  Result:=0;
  if not Assigned(aSocket) then
    aSocket:=FIterator;
...
function TLBaseTcp.SendMessage(const msg: string; aSocket: TLSocket): Integer;
begin
