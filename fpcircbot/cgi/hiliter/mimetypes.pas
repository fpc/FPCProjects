...
uses substrings, SysUtils, StrUtils,
 {$IFDEF UNIX}
   baseunix;
 {$ENDIF}  
 {$IFDEF WIN32}
     ...
     If FileExists(WorkName) then 
       ...
       begin   
        {$IFDEF WIN32}
         ...
         ename := 'WINDIR' + #0;
         SetLength(ConfigDir, 
         GetEnvironmentVariable(@ename[1], @ConfigDir[1],Length(ConfigDir)));
         ConfigDir := ConfigDir + '\';
        {$ENDIF}   
...
begin
  ...
  If (FExtension = '') or (FExtension = '.')  then Exit(DefaultMimeType);
  Delete(FExtension,1,1);  // Delete . 
  FLen := Length(FExtension);
  OpenMimeDataBase;
  If Mime_Stat_Int <> MIME_OK then Exit(DefaultMimeType);
...
INITIALIZATION
// *** We call this here just to make sure we have
// *** A DataBase.
  FindMimeDataBase;
  LastMimeLine := FALSE;
end.  
