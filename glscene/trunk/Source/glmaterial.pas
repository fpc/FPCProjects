...
type
  ...
  ['{8E442AF9-D212-4A5E-8A88-92F798BABFD1}']
   ...
   {: Material in a material library.<p>
	...
	TGLLibMaterial = class (TCollectionItem, IGLMaterialLibrarySupported, IGLNotifyAble, IGLTextureNotifyAble)
	   private
	      { Private Declarations }
         userList : TList;
...
begin
   ...
   if not multitextured then begin
      ...
      if not libMatTexture2.FTextureMatrixIsIdentity then
        libMatTexture2.Material.Texture.ApplyAsTexture2(rci, @libMatTexture2.FTextureMatrix[0][0])
      else
...
{      if Assigned(Material.FTextureEx) then begin
         if not Material.TextureEx.IsTextureEnabled(1) then begin}
            if Assigned(libMatTexture2) and GL_ARB_multitexture and (not vSecondTextureUnitForbidden) then begin
               libMatTexture2.Material.Texture.UnApplyAsTexture2(rci, (not libMatTexture2.TextureMatrixIsIdentity));
