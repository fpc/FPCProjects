...
uses
  ...
  GLObjects, GLMisc, GLCadencer, GLFireFX, ExtCtrls, StdCtrls,
  GLBehaviours, VectorGeometry, GLWin32Viewer, LResources,
  Buttons, GLScene;
...
begin
  //Patch to use in the moment with lazarus.
  //It is not same the real demo I just try to find the problem with the Inertia
  //Because I can add the fire to the sphere but with the Inertia it look to not work good.
  GetOrCreateInertia(Sphere1).DampingEnabled:=True; //not in the demo
  //GetOrCreateFireFX(Sphere1).Manager:=SmokeFX;
  GetOrCreateFireFX(Sphere1).Manager:=FireFX; //just a try to find the problem.
end;
