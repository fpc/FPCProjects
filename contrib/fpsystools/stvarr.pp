 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: Correctness check needed for syntax issue in Sortrows

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StVArr.pas 1.01        *}
{*********************************************************}
...
uses
  Classes, SysUtils,
  StConst, StBase, StUtils; {used for ExchangeStructs}
 ...
 {.Z-}
  ...
  protected
    ...
    function StoresUntypedVars : boolean;
      override;
    ////TL FPC said dupe ID for CacheRows... added 1 suffix
    procedure vmSetCacheRows(CacheRows1 : Integer);
    ...
    ////TL FPC said dupe ID for Rows... added 1 suffix
    procedure vmSetRows(Rows1 : Cardinal);

  {.Z-}
  public
    ////TL FPC claimed dual id declared with rows, cols, Elementsize and CacheRows... added 1 suffix to each
    constructor Create(Rows1, Cols1, ElementSize1 : Cardinal;
                       CacheRows1 : Integer;
    ...
    ////TL FPC claimed dual id declared with Compare... changed to Compare1
    procedure SortRows(KeyCol : Cardinal; Compare1 : TUntypedCompareFunc);
...
////TL FPC claimed dual id declared with rows, cols, Elementsize and CacheRows... added 1 suffix to each
constructor TStVMatrix.Create(Rows1, Cols1, ElementSize1 : Cardinal;
                              CacheRows1 : Integer;
...
////TL FPC claimed dual id declared with Compare... changed to Compare1
procedure TStVMatrix.SortRows(KeyCol : Cardinal; Compare1 : TUntypedCompareFunc);
const
  StackSize = 32;
type
    ...
    try
      ...
      repeat
        ...
        repeat
          ...
          repeat
            Get(PL, KeyCol, CurEl^);
            ////TL!! Removed dereferencing ^
            ////TL!! original: while Compare(CurEl^, PivEl^) < 0 do begin
            ...
            Get(PR, KeyCol, CurEl^);
            ////TL!! Removed dereferencing ^
            ////TL!! original: while Compare(PivEl^, CurEl^) < 0 do begin
...
////TL FPC said dupe ID for CacheRows... added 1 suffix
procedure TStVMatrix.vmSetCacheRows(CacheRows1 : Integer);
var
  I : Integer;
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    if CacheRows1 = FCacheRows then
      Exit;

    if (CacheRows1 < 2) or (CacheRows1 > StMaxBlockSize div SizeOf(TStCacheRec)) then
      RaiseContainerError(stscBadSize);

    {allocate new cache descriptor array}
    GetMem(NewCache, CacheRows1*SizeOf(TStCacheRec));
    FillChar(NewCache^, CacheRows1*SizeOf(TStCacheRec), 0);

    try
      {allocate new buffers if any}
      for I := FCacheRows to CacheRows1-1 do
        with NewCache^[I] do
          HugeGetMem(crRowData, vmRowSize);

      {transfer old cache buffers to new array}
      for I := 0 to FCacheRows-1 do
        if I < CacheRows1 then
      ...
      for I := CacheRows1-1 downto 0 do
        HugeFreeMem(NewCache^[I].crRowData, vmRowSize);
      FreeMem(NewCache, CacheRows1*SizeOf(TStCacheRec));
    end;

    {update cache in-use count}
    if vmCacheCnt > CacheRows1 then
      vmCacheCnt := CacheRows1;

    {deallocate old cache}
    FreeMem(vmCache, FCacheRows*SizeOf(TStCacheRec));
    vmCache := NewCache;
    FCacheRows := CacheRows1;
{$IFDEF ThreadSafe}
  finally
    LeaveCS;
  end;
...
////TL FPC said dupe ID for Rows... added 1 suffix
procedure TStVMatrix.vmSetRows(Rows1 : Cardinal);
var
  I : Integer;
  NewSize : LongInt;
begin
...
{$ENDIF}
    ...
    {validate new size}
    if (Rows1 = 0) or
    ProductOverflow(Rows1, Cols) or
    ProductOverflow(LongInt(Rows1)*LongInt(Cols), FElSize) then
      RaiseContainerError(stscBadSize);

    if Rows1 < FRows then begin
      {dump now-irrelevant rows from cache}
      for I := 0 to vmCacheCnt-1 do
        if vmCache^[I].crRow >= Rows1 then begin
          vmCacheCnt := I;
          break;
        end;
      {truncate data file}
      NewSize := HeaderSize+LongInt(Rows1)*LongInt(Cols)*FElSize;
