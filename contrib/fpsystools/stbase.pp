 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiles but needs a few fixes
   ////TL! this unit has FPC asm issues

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StBase.pas 1.01        *}
{*********************************************************}
...
WideString = AnsiString; WideChar = char; ////TL! remove when widestring support is available

{!!.01 - moved from StStrms.pas }
...
function AnsiUpperCaseShort32(const S : string) : string;
  {-Ansi uppercase for H- strings}

function AnsiCompareTextShort32(const S1, S2: string): Integer;
  {-Case-insensitive compare function for H- strings}

function AnsiCompareStrShort32(const S1, S2: string): Integer;
  {-Case-sensitive compare function for H- strings}
{$ENDIF}
...
function LoWord(l: LongWord): Word;
function LoByte(W: Word): Byte;

function CharUpper(lpsz: PChar): PChar;
function CharLower(lpsz: PChar): PChar;
function IsCharAlpha(lpsz: PChar): Boolean;
function IsCharAlphaW(lpsz: PWideChar): Boolean;
...
begin
  Result := S;
////TL!!! AnsiUpperBuff(PChar(@Result[1]), Length(S));
end;
{$ENDIF}
...
function HugeCompressRLE(const InBuffer; InLen : Longint;
                         var OutBuffer) : Longint; assembler;
...
function HugeDecompressRLE(const InBuffer; InLen : Longint;
                           var OutBuffer; OutLen : LongInt) : Longint; assembler;
...
function ProductOverflow(A, B : LongInt) : Boolean; assembler;
asm
  mov ecx,False
...
//  add    edi, ebx                  { required?  check with debugger }
  xor    ebx, ebx
  jmp    @@Start
...
var
  ...
  LocalMatch : pointer;
begin
  GetMem(LocalMatch, MatLength);
  Move(Match, LocalMatch^, MatLength);
  CharUpper(PChar(LocalMatch));
  GetMem(LocalBuffer, BufLength);
  Move(Buffer, LocalBuffer^, BufLength);
  CharUpper(PChar(LocalBuffer));
  Result := Search(LocalBuffer^, BufLength, LocalMatch^, MatLength, Pos);
  FreeMem(LocalBuffer, BufLength);
  FreeMem(LocalMatch, MatLength);
end;
*)
...
begin
{$IFDEF ThreadSafe}
  ////TL! LibC.InitializeCriticalSection(conThreadSafe);
{$ENDIF}

  FCompare := @AbstractCompare;   ////TL added @ prefix
...
begin
  if conNodeProt = 0 then
    Clear;
{$IFDEF ThreadSafe}
  ////TL! LibC.DeleteCriticalSection(conThreadSafe);
{$ENDIF}
  inherited Destroy;
end;
...
begin
{$IFDEF ThreadSafe}
  ////TL! EnterCriticalSection(conThreadSafe);
{$ENDIF}
end;
...
begin
{$IFDEF ThreadSafe}
  ////TL! LeaveCriticalSection(conThreadSafe);
{$ENDIF}
end;
...
function HiWord(l: LongWord): Word;
begin
  Result := Word(l shr $0F);
end;

function HiByte(W: Word): Byte;
begin
  Result := Word(W shr $08);
end;

function LoWord(l: LongWord): Word;
begin
  Result := Word(l and $0000FFFF);
end;
...
function CharUpper(lpsz: PAnsiChar): PAnsiChar;
{ work-alike function to Windows API CharUpper }
var
  i : LongInt;
begin
  if HiWord(Integer(lpsz)) = 0 then begin { treat it as an in place char }
    ////TL!!! Missing islower: if islower(LoWord(Integer(lpsz))) <> 0 then
     ////TL! lpsz := PAnsiChar(toupper(LoWord(Integer(lpsz))));
  ////TL! end
  ////TL! else begin  { treat it as a null terminated buffer }
    ////TL! for i := 0 to StrLen(lpsz) do begin
      ////TL! lpsz[i] := AnsiChar(toupper(Integer(lpsz[i])));
     ////TL! end;
  end;
  Result := lpsz;
end;

function CharLower(lpsz: PAnsiChar): PAnsiChar;
{ work-alike function to Windows API CharLower }
var
  i : LongInt;
begin
  if HiWord(Integer(lpsz)) = 0 then begin { treat it as an in place char }
    ////TL!!! if isupper(LoWord(Integer(lpsz))) <> 0 then
      ////TL! lpsz := PAnsiChar(tolower(LoWord(Integer(lpsz))));
  ////TL! end
  ////TL! else begin  { treat it as a null terminated buffer }
    ////TL! for i := 0 to StrLen(lpsz) do begin
      ////TL! lpsz[i] := AnsiChar(tolower(Integer(lpsz[i])));
    ////TL! end;
  end;
  Result := lpsz;
end;
...
function IsCharAlpha(lpsz: PChar): Boolean;
{ work-alike function to Windows API IsCharAlpha }
begin
 ////TL! Result := isalpha(Integer(lpsz)) <> 0;
end;

function IsCharAlphaW(lpsz: PWideChar): Boolean;
{ work-alike function to Windows API IsCharAlphaW }
begin
 ////TL!!! Result := isalpha(Integer(lpsz)) <> 0;
end;
