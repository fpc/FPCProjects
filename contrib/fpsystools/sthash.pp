 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: needs a fix for constant evaluation in a declaration.

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StHASH.pas 1.01        *}
{*********************************************************}
...
{.Z+}
  ////TL!!! FPC can't evaluate constant expr: THashArray = array[0..(MaxInt div SizeOf(TStHashNode))-1] of TStHashNode;
  THashArray = array[0..1000] of TStHashNode; ////TL!!! haywire to get the rest of unit compiled
...
{$ENDIF}
    ...
    if (Source is TStHashTable) then begin
      ...
      TStHashTable(Source).Iterate(@AssignNode, Self); ////TL Added @
    end
    else
...
{$ENDIF}
    if FCount <> 0 then begin
      Iterate(@DestroyNode, nil); ////TL Added @
      FCount := 0;
      htLRU := 0;
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    htIterate(@FindNodeData, Data, H, P, T);  ////TL Added @
    if Assigned(T) then begin
      htMoveToFront(H, P, T);
      htUpdateLRU(T);
      Result := True;
...
begin
  ...
  if htLRU = MaxLongInt then begin
    ...
    repeat
      ...
      MinNode.MNode := nil;
      Iterate(@FindMinPositiveNode, @MinNode);  ////TL Added @
    ...
    Iterate(@NegateNodeLRU, nil);  ////TL Added @
  end;
end;
...
{$ENDIF}
    ...
    H.Iterate(@JoinNode, Self);  ////TL Added @
      ...
      with Reader do begin
        ...
        try
          ...
          while not EndOfList do begin
            ReadBoolean;
            //Read(Value^, FValSize); ////!!!!////TL FPC won't accept the pointer reference
        ...
        if Count <> 0 then
          for H := 0 to FHashSize-1 do begin
            ...
            while Assigned(Walker) do begin
              ...
              WriteBoolean(True);
              //Write(Walker.Value^, FValSize); ////!!!!////TL FPC won't accept the pointer reference
