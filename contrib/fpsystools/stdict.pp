...
{*********************************************************}
{* TurboPower SysTools for Kylix: StDict.pas 1.01        *}
{*********************************************************}
...
{Notes:
  ...
  In 32-bit mode, there is no practical limit on HashSize.
  A particular value of HashSize may lead to a better distribution
  of symbols in the dictionary, and therefore to better performance.
  Generally HashSize should be about the same size as the number of
  symbols expected in the dictionary. A prime number tends to give a
  better distribution.
...
MaxBytes = 128*1024*1024; // Added by TL

type
  TByteArray = ARRAY [0..MaxBytes-1] Of Byte; // Added by TL
...
{.Z-}
    public
      // Duplicate name declaration error in FPC... changed to Name1  TL
      constructor CreateStr(const Name1 : string; AData : Pointer);
        {-Initialize node}
...
{.Z+}
  ////TL!!!: FPC can't evaluate the constant expression
  ////TL!!! TSymbolArray = array[0..(StMaxBlockSize div SizeOf(TStDictNode))-1] of TStDictNode;
  TSymbolArray = array[0..1000] of TStDictNode; ////TL!!! temp haywire to get the unit to compile
...
begin
{$IFDEF ThreadSafe}
  ////TL! EnterCriticalSection(ClassCritSect);
{$ENDIF}
end;
...
begin
{$IFDEF ThreadSafe}
  ////TL! LeaveCriticalSection(ClassCritSect);
{$ENDIF}
end;
...
// Duplicate name declaration error in FPC... changed to Name1  TL
constructor TStDictNode.CreateStr(const Name1 : string; AData : Pointer);
begin
  Create(AData);
  {dnNext := nil;}
  {$IFDEF HStrings}
...
function AnsiHashStr(const S : string; Size : Integer) : Integer; Assembler;
{$IFDEF HStrings}
  {32-bit huge string}
asm
  push ebx
...
procedure TStDictionary.Assign(Source: TPersistent);
  ...
  begin
    ...
    if (Source is TStDictionary) then
      begin
        Clear;
        TStDictionary(Source).Iterate(@AssignNode, Self); ////TL Added @ prefix
...
{$ENDIF}
    if FCount <> 0 then begin
      Iterate(@DestroyNode, nil); ////TL Added @ prefix
      FCount := 0;
...
{$IFDEF WStrings}
  FEqual := @AnsiCompareTextShort32;   ////TL: added "@" sign prefix
{$ELSE}
  FEqual := AnsiCompareText;
{$ENDIF}
  FHash := @AnsiHashText; ////TL: added "@" sign prefix
  HashSize := AHashSize;
end;
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    T := Iterate(@FindNodeData, Data);   ////TL: added "@" sign prefix
    if Assigned(T) then begin
      Result := True;
  {$IFDEF HStrings}
      Name := T.dnName;
...
{$ENDIF}
    if FCount <> 0 then begin
      for H := 0 to FHashSize-1 do begin
        T := dySymbols^[H];
        while Assigned(T) do begin
          S.AddObject(T.Name, TObject(T.Data)); ////TL!! added TObject typecast
          T := T.dnNext;
        end;
      end;
    end;
...
{$ENDIF}
    ...
    D.Iterate(@JoinNode, Self); ////TL added @ prefix
...
{$IFDEF ThreadSafe}
initialization
  ////TL! InitializeCriticalSection(ClassCritSect);
finalization
  ////TL! DeleteCriticalSection(ClassCritSect);
{$ENDIF}
end.
