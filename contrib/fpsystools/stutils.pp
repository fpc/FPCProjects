 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiles ok but 2 problem asm routines in this unit
   ////TL! this unit has FPC asm issues

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StUtils.pas 1.01       *}
{*********************************************************}
{* SysTools: Low level data manipulations                *}
{*********************************************************}
...
function TrimLead(const S : ShortString) : ShortString; overload;
...
procedure ExchangeLongInts(var I, J : LongInt); assembler;
asm
  mov  ecx, [eax]
  push ecx
  mov  ecx, [edx]
...
procedure ExchangeStructs(var I, J; Size : Cardinal); assembler;
asm
  push edi
  push ebx
  push ecx
...
procedure FillWord(var Dest; Count : Cardinal; Filler : Word);  assembler;
asm
  push  edi
  mov   edi,Dest
  mov   ax,Filler
...
function MidFloat(F1, F2, F3 : Extended) : Extended;
  {return the middle of three floating point values}
begin
  Result := MinFloat(MinFloat(MaxFloat(F1, F2), MaxFloat(F2, F3)),
MaxFloat(F1, F3)); end;
