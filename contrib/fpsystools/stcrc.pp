 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiled ok without changes... lightly tested

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StCRC.pas 1.01         *}
{*********************************************************}
...
begin
  ...
  for I := 0 to (DataSize-1) do
    Result := Crc32Table[Byte(Result xor CRCByteArray(Data)[I])] xor
      LongWord((Result shr 8) and $00FFFFFF);
end;
...
begin
  FileSt := TFileStream.Create(FileName, CrcFileMode);
  try
    Result := Crc32OfStream(FileSt, LongInt($FFFFFFFF));
  finally
    FileSt.Free;
  end;
end;
