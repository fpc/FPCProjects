 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiles ok... ststr asm needs to be fixed
   ////TL! but Initialization block generates segfault from ststr asm call

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StDateSt.pas 1.01      *}
{*********************************************************}
...
function DateStringHMStoAstJD(Picture, DS : string; H,M,S,Epoch : integer) : Double;
...
function InternationalDate(ForceCentury : Boolean) : string;
  {-Return a picture mask for a short date string, based on Kylix international
    information}

function InternationalLongDate(ShortNames : Boolean;
                               ExcludeDOW : Boolean) : string;
  {-Return a picture mask for a date string, based on Kylix international
    information}

function InternationalTime(ShowSeconds : Boolean) : string;
  {-Return a picture mask for a time string, based on Kylix international
    information}

procedure ResetInternationalInfo;
  {-Update internal info to match Kylix international info}
...
var
  ...
  procedure ExtractFromPicture(Picture, S : string; Ch : ANSIChar; var I : Integer;
  ...
  function MonthStringToMonth(MSt : string; Width : Byte) : Byte;
  ...
  procedure ExtractFromPicture(Picture, S : string;
  ...
  function PackResult(Picture, S : string) : string;
  ...
  procedure MergePictureSt(Picture : string; var P : string;
                          MC : ANSIChar; SP : string);
  var
    I, J : Cardinal;
    L    : Cardinal;
  begin
  ...
  function TimeToTimeStringPrim(Picture : string; T : TStTime;
                                Pack : Boolean; t1159, t2359 : string) : string;
  ...
  begin
    ...
    if OK then begin
      if (Length(t1159) = 0) and (Length(t2359) = 0) then begin
        ...
        I := 1;
        L := Length(P);
        while (I <= L) and (Result[TPos] = TimeOnly) do begin            {!!.01}
          Result[TPos] := P[I];
          Inc(I);
          Inc(TPos);
        end;
  ...
  function MaskCharCount(P : string; MC : ANSIChar) : Integer;
  var
    I, R,                                                              
    Len  : Cardinal;
    OK   : Boolean;
  ...
  function InternationalDate(ForceCentury : Boolean) : string;
    {-Return a picture mask for a date string, based on Kylix int'l info}
  ...
  begin
    {copy Kylix mask into temporary var}
    Temp := wLongDate;
  ...
  function InternationalTime(ShowSeconds : Boolean) : string;
    {-Return a picture mask for a time string, based on Kylix int'l info}
  var
    ML,
    I  : Integer;
  begin
...
initialization
  {initialize DefaultYear and DefaultMonth}
  SetDefaultYear;
  ////TL!! looks like a register parm or syntax problem with the StrChPosS assembler code
  ////TL!! calling this segfaults asm routines in ststr:   ResetInternationalInfo;
end.
