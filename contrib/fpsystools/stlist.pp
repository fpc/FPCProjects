 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiles ok with minor changes... ready for testing

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StList.pas 1.01        *}
{*********************************************************}
...
type
  ...
  {.Z+}
    ////TL! Replaced Protected with Public. Protected caused a LOT of errors with
    ////TL! FPC in stcoll when the protected FPREV: N := TPageDescriptor(N.FPrev);
    ////TL! was passed as a parameter. This should have caused an error... why does
    ////TL! Delphi compile it?
    public ////TL! declaring the fields public... not the best practice but we can fix it later
...
procedure TStList.Assign(Source: TPersistent);
  begin
    ...
    {$ENDIF}
      ...
      if not AssignPointers(Source, @AssignData) then  ////TL: Added "@" sign to AssignData
...
{$ENDIF}
    if Count > 0 then begin
      Iterate(@DestroyNode, True, nil);  ////TL: Added "@" sign to AssignData
      FCount := 0;
    end;
    FHead := nil;
    FTail := nil;
...
{$IFDEF ThreadSafe}
  ...
  try
{$ENDIF}
    Result := Iterate(@FindNode, True, Data); ////TL: Added "@" sign to AssignData
{$IFDEF ThreadSafe}
  finally
    LeaveCS;
  end;
