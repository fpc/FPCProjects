 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: Lots of FPC compiler issues and needed fixes... unusable

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StExpr.pas  1.01       *}
{*********************************************************}
...
uses SysUtils, Classes, {$IFDEF UseMathUnit} Math, {$ENDIF} StBase, StConst, StMath;
...
type
  ...
  protected {private}
    ...
    {ident list routines}
    function FindIdent(Name1 : string) : Integer; ////TL Dupe ID: Name... added 1 suffix
  ...
  {.Z-}
    ...
    ////TL Dupe ID in the following: Name... added 1 suffix
    function AnalyzeExpression : TStFloat;
    procedure AddConstant(const Name1 : string; Value : TStFloat);
    procedure AddFunction0Param(const Name1 : string; FunctionAddr : TStFunction0Param);
    procedure AddFunction1Param(const Name1 : string; FunctionAddr : TStFunction1Param);
    procedure AddFunction2Param(const Name1 : string; FunctionAddr : TStFunction2Param);
    procedure AddFunction3Param(const Name1 : string; FunctionAddr : TStFunction3Param);
    procedure AddInternalFunctions;
    procedure AddMethod0Param(const Name1 : string; MethodAddr : TStMethod0Param);
    procedure AddMethod1Param(const Name1 : string; MethodAddr : TStMethod1Param);
    procedure AddMethod2Param(const Name1 : string; MethodAddr : TStMethod2Param);
    procedure AddMethod3Param(const Name1 : string; MethodAddr : TStMethod3Param);
    procedure AddVariable(const Name1 : string; VariableAddr : PStFloat);
    procedure ClearIdentifiers;
    procedure GetIdentList(S : TStrings);
    procedure RemoveIdentifier(const Name1 : string);
...
procedure TStExpression.AddConstant(const Name1 : string; Value : TStFloat);
var
  IR : PStIdentRec;
begin
...
procedure TStExpression.AddFunction0Param(const Name1 : string;
...
procedure TStExpression.AddFunction1Param(const Name1 : string;
          FunctionAddr : TStFunction1Param);
var
  IR : PStIdentRec;
begin
  if FindIdent(Name1) > -1 then
    RaiseExprError(stscExprDupIdent, 0);
...
procedure TStExpression.AddFunction2Param(const Name1 : string;
...
procedure TStExpression.AddFunction3Param(const Name1 : string;
...
procedure TStExpression.AddInternalFunctions; ////TL added @ to each of the passed function parms
begin
  eBusyFlag := True;
  try
...
procedure TStExpression.AddMethod0Param(const Name1 : string;
...
procedure TStExpression.AddMethod1Param(const Name1 : string;
...
procedure TStExpression.AddMethod2Param(const Name1 : string;
...
procedure TStExpression.AddMethod3Param(const Name1 : string;
...
procedure TStExpression.AddVariable(const Name1 : string; VariableAddr : PStFloat);
var
  IR : PStIdentRec;
begin
  if FindIdent(Name1) > -1 then
...
begin
  for I := 0 to eIdentList.Count-1 do
    ////TL!!! Dispose(PStIdentRec(eIdentList[I]));
  eIdentList.Clear;
end;
...
function TStExpression.FindIdent(Name1 : string) : Integer;
var
  I : Integer;
begin
  Result := -1;
  for I := 0 to eIdentList.Count-1 do begin
    if Name1 = PStIdentRec(eIdentList[I])^.Name then begin
      Result := I;
      Break;
    end;
  end;
    ...
    case Ident^.Kind of
      ...
      ikFunction :
        begin
          ...
          try
             ////TL!!! Parm passing type issue with all cases
...
procedure TStExpression.RemoveIdentifier(const Name1 : string);
var
  I : Integer;
  S : string;
begin
