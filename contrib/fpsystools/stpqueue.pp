...
{*********************************************************}
{* TurboPower SysTools for Kylix: StPQueue.pas 1.01      *}
{*********************************************************}
{* SysTools: Priority queue class                        *}
{*********************************************************}
...
{Notes:
   ...
   In 32-bit programs the limit is set by memory usage or performance.
}
...
begin
  ...
  try
  {$ENDIF}
    if not AssignPointers(Source, @AssignData) then ////TL added @
...
{$ENDIF}
    if conNodeProt = 0 then
      ForEachPointer(@StPQueue.DestroyNode, nil); ////TL Added @
    FCount := 0;
{$IFDEF ThreadSafe}
  finally
    LeaveCS;
...
{$ENDIF}
    ...
    Q.ForEachPointer(@JoinData, Self); ////TL Added @
    Q.IncNodeProtection;
    Q.Free;
{$IFDEF ThreadSafe}
  finally
...
{$ENDIF}
    ...
    try
      with Writer do begin
        ...
        Iterate(@StoreNode, @StoreInfo); ////TL Added @
        WriteListEnd;
      end;
    finally
      Writer.Free;
