 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiled cleanly with no changes... ready for testing with Pascal inline
   ////TL! this unit has FPC asm issues BUT has an IFDEF option to use a pascal implementation

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StBCD.pas 1.01         *}
{*********************************************************}
{* SysTools: BCD (Binary Coded Decimal)                  *}
...
////TL FPC Conversion notes: 15-6-2003 23:05
////TL Added an Assembler reserved word to inline assembler routines.
////TL These routines are ifdef'ed with a native Pascal implementation... so
////TL these may need to be removed when comiling on other platforms.
...
procedure AddMantissas(const UB1 : TUnpBcd; var UB2 : TUnpBcd); Assembler;
{$IFDEF UseAsm}
  asm
    push esi
    push edi
...
function IsZeroMantissa(const UB : TUnpBcd) : Boolean;  Assembler;
{$IFDEF UseAsm}
 asm
   push edi
   mov edi,UB
...
procedure NegMantissa(var UB : TUnpBcd);  Assembler;
{$IFDEF UseAsm}
  asm
    push edi
    mov edi,UB
...
procedure SubMantissas(const UB1 : TUnpBcd; var UB2 : TUnpBcd);  Assembler;
{$IFDEF UseAsm}
  asm
    push esi
    push edi
