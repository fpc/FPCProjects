 ...
 * ***** END LICENSE BLOCK ***** *)
 ...
 Tom Lisjac <vlx@users.sourceforge.net> http://theseus.sf.net
   ...
   17-6-2003 status: compiles ok... needs a few fixes
   ////TL! This unit has FPC asm issues... one bad line that won't even compile

 *)

{*********************************************************}
{* TurboPower SysTools for Kylix: StSort.pas 1.01        *}
{*********************************************************}
{* SysTools: General purpose sorting class               *}
...
type
  ...
  TMergeInfo = record          {Record returned by MergeInfo}
    SortStatus   : Integer;    {Predicted status of sort, assuming disk ok} {!!.01}
 ...
 {$IFDEF ThreadSafe}
    ////TL! sorThreadSafe  : TRTLCriticalSection;{Windows critical section record}
 {$ENDIF}
  ...
  public
    ////TL Dupe id: RecLen... added 1 suffix
    constructor Create(MaxHeap : LongInt; RecLen1 : Cardinal); virtual;
      {-Initialize a sorter}
...
const
  {ecOutOfMemory = 'Out of Memory';}                                     {!!.01}
  ecOutOfMemory = 8;                                                     {!!.01}

procedure RaiseError(Code : Integer);                                    {!!.01}
var
  E : ESTSortError;
begin
  if Code = ecOutOfMemory then
    ...
    E := ESTSortError.CreateResTP(Code, 0);                              {!!.01}
    E.ErrorCode := Code;                                                 {!!.01}
    raise E;
  end;
end;
...
function MergeInfo(MaxHeap : LongInt; RecLen : Cardinal;
                   NumRecs : LongInt) : TMergeInfo;
type
  MergeFileSizeArray = array[1..(StMaxBlockSize div SizeOf(Cardinal))] of Cardinal;
var
...
{ -- BOGUS "Variable might not have been initialized error" }
{ Bug in Kylix 1 Code analysis fixed in Kylix 2 }
...
////TL Dupe id: RecLen... added 1 suffix
constructor TStSorter.Create(MaxHeap : LongInt; RecLen1 : Cardinal);
begin
  ...
  FMergeName := @DefaultMergeName; ////TL Added @ suffix
  FRecLen := RecLen1;
...
{$IFDEF ThreadSafe}
  ////TL! InitializeCriticalSection(sorThreadSafe);
{$ENDIF}
end;
...
begin
{$IFDEF ThreadSafe}
  ////TL! DeleteCriticalSection(sorThreadSafe);
{$ENDIF}
  sorDeleteMergeFiles;
  sorFreeBuffer;
end;
...
begin
{$IFDEF ThreadSafe}
  ////TL! EnterCriticalSection(sorThreadSafe);
{$ENDIF}
end;
...
begin
{$IFDEF ThreadSafe}
  ////TL! LeaveCriticalSection(sorThreadSafe);
{$ENDIF}
end;
...
//    sorBuffer := GlobalAllocPtr(HeapAllocFlags, AllocRecs*LongInt(FRecLen));
    SorBufSize := AllocRecs*LongInt(FRecLen);
...
asm
  ...
  mov edx,edi
  mov edi,Dest
  ////TL!!! mov ecx,TStSorter([eax]).FRecLen
  mov eax,ecx
  shr ecx,2
  rep movsd
  mov ecx,eax
...
type
  Stack = array[0..StackSize-1] of Cardinal;
var
  Pl, Pr, Pm, L, R : Cardinal;
  ArraySize, PartitionLen : Cardinal;
  PivotPtr : Pointer;
  SwapPtr : Pointer;
  StackP : Integer;
  Lstack, Rstack : Stack;
  ...
  procedure SwapElements(L, R : Cardinal);
  var
    LPtr : Pointer;
    RPtr : Pointer;
  begin
