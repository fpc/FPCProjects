...
#define SP_REGNUM 4	/* (esp) Contains address of top of stack */
...
#define FPU_REAL_SIZE 10

/* Total amount of space needed to store our copies of the machine's register
   state, the array `registers'. */

#define REGISTER_BYTES ((NUM_REGS - NUM_FREGS)*4 + NUM_FREGS*FPU_REAL_SIZE)

/* Index within `registers' of the first byte of the space for register N. */

#define REGISTER_BYTE(N) \
  (((N) < FP0_REGNUM) ? ((N) * 4) : ((((N) - FP0_REGNUM) * FPU_REAL_SIZE) + 64))
 
/* Number of bytes of storage in the actual machine representation for
   register N.  All registers are 4 bytes, except 387 st(0) - st(7),
   which are 80 bits each. */

#define REGISTER_RAW_SIZE(N) (((N) < FP0_REGNUM) ? 4 : FPU_REAL_SIZE)

/* Largest value REGISTER_RAW_SIZE can have.  */

#define MAX_REGISTER_RAW_SIZE FPU_REAL_SIZE

/* Number of bytes of storage in the program's representation
   for register N. */

#define FPUREG_VIRTUAL_SIZE 12

#define REGISTER_VIRTUAL_SIZE(N) (((N) < FP0_REGNUM) ? 4 : FPUREG_VIRTUAL_SIZE)

/* Largest value REGISTER_VIRTUAL_SIZE can have.  */

#define MAX_REGISTER_VIRTUAL_SIZE FPUREG_VIRTUAL_SIZE 
...
/* long double only use 10 bytes
   but are aligned to 12 bytes in GNU C
   FPC uses 10 instead */
#define ALTERNATE_LONG_DOUBLE_SIZE 10

#define REGISTER_VALID(i) ( (i < 16) ? 1 : fpue.isvalid[i-16])

#define DO_REGISTERS_INFO(regnum, fp) if (!fp) do_registers_info(regnum, fp); \
        else { \
        int i;\
        int numregs = ARCH_NUM_REGS;\
        char raw_buffer[MAX_REGISTER_RAW_SIZE]; \
        char virtual_buffer[MAX_REGISTER_VIRTUAL_SIZE]; \
        do_registers_info(-1,0); \
        printf_filtered ("\tFPU stack\n"); \
        for (i=16;i<numregs;i++) \
         if ((fpue.isvalid[i-16]) && ((i==regnum) || (regnum==-1)))\
          { int j;\
